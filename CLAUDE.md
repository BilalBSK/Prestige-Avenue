...
- **Media storage**: Cloudflare R2 (S3-compatible, free unlimited egress) for images and short videos. Admin upload flow is client-direct: browser requests a presigned PUT URL from `/api/admin/upload` (CSRF + admin guard), then PUTs the file to R2. The presign signs only `content-type` and `content-length` (keep it minimal — the bucket CORS policy must allow every signed header, so don't add `cache-control` here unless the bucket CORS is updated in lockstep). Caching is configured at the edge via a Cloudflare Cache Rule on the custom domain, NOT per-object at upload time. Public reads go through `R2_PUBLIC_BASE_URL`, which **must be a custom Cloudflare domain in production** (the `pub-*.r2.dev` subdomain is dev-only: rate-limited, no CDN cache); the preview deployment keeps using `r2.dev`. Caps: images 5 MB, videos 50 MB; allowed MIMEs in `src/lib/blob.ts`, enforced on the presign route. Production setup runbook: `docs/r2-production-setup.md`.
